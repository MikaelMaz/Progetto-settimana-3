...
let stringa1 = "Peppe"
let booleano = false
let cancellaMale = deleteOne(stringa1 , booleano)
console.log(cancellaMale)
...
function onlyLetters(stringa) {
  let conspazio =  stringa.replace(/[0-9]/g, '');
  return conspazio.split('  ').join(' ')
}

let cani = "I have 27282 dogs"
let togliNumeri = onlyLetters(cani)
...
function howManyDays(data) {
  ...
  let differenzaGiorni = Math.abs(Math.floor(differenzaMillisecondi / (1000 * 60 * 60 * 24)));
...
function isTodayMyBirthday() {
  let oggi = new Date();
  let comply = new Date(oggi.getFullYear(), 4, 20);
...
let propEliminata = deleteProp(obj, 'nome');
console.log(obj);
...
function onlyInLastMillennium(movies) {
  return movies.filter(movie => parseInt(movie.Year) <= 2000).map(movie => (
    {Title: movie.Title,
    Year: movie.Year,})
    );
}

let Millennio = onlyInLastMillennium(movies)
console.log ("scorso millennio:", Millennio)
...
let risultatiRicerca = searchByTitle(movies, "Lord");
...
*/
function searchAndDivide(movies, cerca) {
  let match = [];
  let unmatch = [];

  movies.forEach(movie => {
    if (movie.Title.toLowerCase().includes(cerca.toLowerCase())) {
      match.push(movie);
    } else {
      unmatch.push(movie);
    }
...
const moviesSenza = removeIndex(movies, 2);
console.log(moviesSenza);
...
function stampatd() {
  tutti = tuttitd()
  tutti.forEach(td => console.log(td.textContent))
}
...
function sfondoLink() {
  let tutti = document.querySelectorAll('a')
  tutti.forEach(link => link.style.backgroundColor= 'green')
}
...
function nuovoLi() {
  ...
  nuovo.textContent = 'Non lo so'
  document.getElementById('myList').appendChild(nuovo)
}
